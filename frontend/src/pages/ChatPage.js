  ...
  const [messages, setMessages] = useState([
    {
      id: '1',
      type: 'ai',
      content: "Hello! I'm EcoSphere's environmental AI assistant. I can help you understand air quality, weather conditions, environmental health tips, and answer questions about your local environment. How can I assist you today?",
      timestamp: new Date()
    }
  ]);
  ...
  const handleSendMessage = async (messageText = inputMessage) => {
    ...
    try {
      ...
      const errorMessage = {
        id: (Date.now() + 1).toString(),
        type: 'ai',
        content: "I apologize, but I'm experiencing some technical difficulties right now. Please try again in a moment, or feel free to explore the environmental data on the main page.",
        timestamp: new Date()
      };
      ...
      <div className="max-w-4xl mx-auto px-4 h-[calc(100vh-7rem)] flex flex-col">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Ask me anything about environmental conditions, air quality, weather, and eco-friendly tips
          </p>
          {currentLocation && (
            <div className="mt-3">
              <Badge className="bg-emerald-100 text-emerald-700">
                <MapPin className="h-3 w-3 mr-1" />
                Location detected: {currentLocation.address || `${currentLocation.latitude.toFixed(4)}, ${currentLocation.longitude.toFixed(4)}`}
              </Badge>
            </div>
          )}
        </div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-sm font-medium ${
                        message.type === 'user' ? 'text-emerald-100' : 'text-gray-600'
                      }`}>
                        {message.type === 'user' ? 'You' : 'EcoSphere AI'}
                      </span>
                      <span className={`text-xs ${
                        message.type === 'user' ? 'text-emerald-200' : 'text-gray-400'
                      }`}>
