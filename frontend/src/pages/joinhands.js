...
// Ensure cookies are sent with every request for auth
    ...
    <div className="relative overflow-hidden bg-white rounded-2xl border border-green-100 shadow-md hover:shadow-xl transition-all duration-300 group">
      <div className="flex flex-col md:flex-row">
        <div className="relative w-full md:w-48 h-48 md:h-auto overflow-hidden">
          <img 
            src={drive.image_url || "https://images.unsplash.com/photo-1532996122724-e3c354a0b15b?auto=format&fit=crop&q=80"} 
            alt={drive.title} 
          ...
          <div className="absolute top-3 right-3 bg-yellow-100 text-yellow-800 border-2 border-yellow-300 px-3 py-1.5 rounded-full font-bold text-xs flex items-center gap-1 shadow-md">
            <Award className="w-4 h-4" /> Earn {pointsReward} pts
          </div>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-4">
              <div className="flex items-center gap-2 bg-blue-50 px-3 py-1.5 rounded-full border border-blue-200">
                ...
                <span className="text-sm font-semibold text-blue-700">
                  {currentParticipants} Volunteers Joined
                </span>
              </div>
...
export default function JoinHands() {
  ...
  const handleJoinDrive = async (driveId, drivePoints) => {
    try {
  ...
  const handleRedeemReward = async (rewardId, cost) => {
    if (userStats.points < cost) return alert("Not enough points!");
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center gap-4 mt-8">
            <Button onClick={() => setActiveTab("join")} variant={activeTab === "join" ? "default" : "outline"} className={`rounded-full px-8 ${activeTab === "join" ? "bg-[#2E7D32]" : ""}`}>Explore Drives</Button>
            <Button onClick={() => setActiveTab("start")} variant={activeTab === "start" ? "default" : "outline"} className={`rounded-full px-8 ${activeTab === "start" ? "bg-[#2E7D32]" : ""}`}>Start a Movement</Button>
          </div>
        </div>
            ...
            <div className="bg-white rounded-3xl border border-green-100 shadow-sm p-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-4">
                <div className="md:col-span-8 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                  <div className="flex justify-between mb-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-slate-500">🟡 {userStats.rankTitle}</p>
                      </div>
                    </div>
                  ...
                  <Progress value={userStats.progressPercentage} className="h-3 bg-green-50" />
                  <p className="text-xs text-slate-400 mt-2">{userStats.progressPercentage}% progress to next level</p>
                </div>
                <div className="md:col-span-4 bg-[#2E7D32] p-6 rounded-2xl text-white flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-4xl font-black">#{userStats.globalRank}</p>
                    <p className="text-xs opacity-70">Keep earning to climb!</p>
                  </div>
            ...
            <div className="pt-12 border-t border-slate-200">
              ...
              {rewards.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {rewards.map((reward) => (
                    <div key={reward._id} className="bg-white rounded-2xl border-2 p-6 border-slate-100 shadow-sm hover:shadow-md transition-all">
                      <div className="flex flex-col items-center text-center">
                        <div className="bg-green-50 p-4 rounded-full mb-4 text-[#2E7D32]">
                           {reward.category === 'Discount' ? <Tag /> : reward.category === 'Premium' ? <Star /> : <Leaf />}
                        </div>
                        ...
                        <Button 
                          ...
                          className={`w-full rounded-full ${userStats.points < reward.cost ? 'bg-slate-300' : 'bg-[#2E7D32]'}`}
                        >
                          {reward.cost} pts - Redeem
                        </Button>
                      </div>
                    </div>
                  ))}
                ...
                <p className="text-center text-slate-400">No rewards available at the moment.</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="flex justify-center animate-in slide-in-from-bottom-4 duration-500">
            <Card className="shadow-2xl border-none bg-white max-w-lg w-full rounded-3xl overflow-hidden">
              <CardHeader className="bg-[#2E7D32] text-white p-8">
                <CardTitle className="text-3xl font-bold flex items-center gap-2">🧹 Start a Movement</CardTitle>
              ...
              <CardContent className="p-8 space-y-4">
                ...
                <Button className="w-full bg-[#2E7D32] hover:bg-[#1B5E20] py-6 text-lg font-bold rounded-2xl" onClick={handleCreateDrive}>Launch Drive</Button>
              </CardContent>
            </Card>
          </div>
        )}
